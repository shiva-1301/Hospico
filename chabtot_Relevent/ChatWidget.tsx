...
const ChatWidget = ({ autoOpen = false, embedMode = false }: ChatWidgetProps) => {
        ...
        if (typeof window !== 'undefined') {
            ...
            if (SpeechRecognition) {
                ...
                recognitionRef.current.onresult = (event: any) => {
                    ...
                    // Check if we're in voice booking mode
                    if (voiceBookingActive || currentBookingStep) {
                        handleVoiceBookingCommand(transcript);
                        // Keep listening in booking mode
                        setTimeout(() => {
                            if (voiceBookingActive && recognitionRef.current) {
                                recognitionRef.current.start();
                            }
                        }, 500);
                    } else {
                        // Normal mode: add to input field  
    ...
    useEffect(() => {
        console.log('[Voice Booking] Messages changed, count:', messages.length);
        if (messages.length > 0) {
            const lastMessage = messages[messages.length - 1];
            console.log('[Voice Booking] Last message role:', lastMessage.role, 'step:', lastMessage.step);

            if (lastMessage.role === 'bot' && lastMessage.step) {
                console.log('[Voice Booking] Setting current step to:', lastMessage.step);
                setCurrentBookingStep(lastMessage.step);

                // Enable voice booking for these steps
                const voiceEnabledSteps = ['hospital_selection', 'doctor_selection', 'time_selection', 'patient_details'];
                if (voiceEnabledSteps.includes(lastMessage.step)) {
                    console.log('[Voice Booking] Enabling voice booking mode');
                    setVoiceBookingActive(true);

                    // Auto-start mic and announce options
                    setTimeout(() => {
                        console.log('[Voice Booking] Attempting to start microphone, isListening:', isListening);
                        if (recognitionRef.current) {
                            try {
                                if (isListening) {
                                    console.log('[Voice Booking] Mic already listening, stopping first');
                                    recognitionRef.current.stop();
                                }
                                ...
                                setTimeout(() => {
                                    try {
                                        console.log('[Voice Booking] Starting microphone now');
                                        recognitionRef.current.start();
                                        setIsListening(true);
                                        console.log('[Voice Booking] Microphone started successfully');

                                        // Announce available options
                                        switch (lastMessage.step) {
                                            case 'hospital_selection':
                                                if (lastMessage.hospitals && lastMessage.hospitals.length > 0) {
                                                    const announcement = `Please say the number of your choice from 1 to ${lastMessage.hospitals.length}`;
                                                    speakMessage(announcement, -1);
                                                }
                                                break;
                                            case 'doctor_selection':
                                                if (lastMessage.doctors && lastMessage.doctors.length > 0) {
                                                    const announcement = `Please say the number of your choice from 1 to ${lastMessage.doctors.length}`;
                                                    speakMessage(announcement, -1);
                                                }
                                                break;
                                            case 'time_selection':
                                                if (lastMessage.availableSlots && lastMessage.availableSlots.length > 0) {
                                                    const announcement = `Please say the number of your choice from 1 to ${lastMessage.availableSlots.length}`;
                                                    speakMessage(announcement, -1);
                                                }
                                                break;
                                            case 'patient_details':
                                                speakMessage('Say yes or confirm to proceed with booking', -1);
                                                break;
                                        }
                                    } catch (e) {
                                        console.error('[Voice Booking] Failed to start mic:', e);
                                    }
                                }, 200);
                            } catch (e) {
                                console.error('[Voice Booking] Error in mic start logic:', e);
                            }
                        } else {
                            console.error('[Voice Booking] No recognition ref available');
                        }
                    }, 1500);
                } else if (lastMessage.step === 'booking_confirmed') {
                    // Booking complete, disable voice mode
    ...
    const handleVoiceBookingCommand = (transcript: string) => {
        console.log('[Voice Booking] Received:', transcript, 'Step:', currentBookingStep);

        if (!currentBookingStep) {
            // Not in booking mode, treat as normal input
            setInput(prev => prev + ' ' + transcript);
            return;
        }

        const command = parseVoiceCommand(transcript, currentBookingStep);
        console.log('[Voice Booking] Parsed command:', command);
        ...
        // Get last message with booking data
        const lastMessage = messages[messages.length - 1];
        ...
        switch (currentBookingStep) {
            ...
            case 'date_selection':
                if (command.type === 'date') {
                    ...
                    // Allow selecting by number of days from today
                    const date = new Date();
            ...
            case 'time_selection':
                if (command.type === 'number' && lastMessage.availableSlots) {
                    ...
                    // Try to match the spoken time to available slots
